...
import com.fyp.eshop.databinding.ActivityProductInfoBinding;
...
//    double unit_price;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_eleg = binding.btnEleg;
        tv_total_price = binding.tvTotalPrice;
        btn_addToCart = binding.btnAddToCart;
        productId = getIntent().getStringExtra("productId");

//        Log.i("Product ID",productId);

        btn_eleg.setRange(1,10);
        btn_eleg.setOnClickListener(new ElegantNumberButton.OnClickListener() {
            @Override
            public void onClick(View view) {
                String num = btn_eleg.getNumber();
                Log.i("Number", num);
            }
        });


        btn_eleg.setOnValueChangeListener(new ElegantNumberButton.OnValueChangeListener() {
            @Override
            public void onValueChange(ElegantNumberButton view, int oldValue, int newValue) {
                Log.d("Tag", String.format("oldValue: %d   newValue: %d", oldValue, newValue));
                qty = newValue;
                tv_total_price.setText("Total $" +  unit_price * qty);
            }
        });
        ...
        btn_addToCart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(qty <= 0) {
                    Toast.makeText(getApplicationContext(),"Please select Quantity > 0",Toast.LENGTH_SHORT).show();
                    return;
                }
                addToCartNew();
            }
...
//                });
    ...
    private void setProductInfo() {
        Glide.with(ProductInfoActivity.this).load(entity.getProductThumb()).placeholder(R.drawable.loading_shape)
                .dontAnimate().into(img_thumb);
    ...
    private void addToCartNew() {
        ...
        if(entity.getStock()<=0){
            Toast.makeText(this,"Sorry! Product Out Of Stock.", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        data.put("productDocId",docId);
        data.put("productId",productId);
        data.put("qty",qty);
        data.put("createDt", new Timestamp(new Date()));

        firebase.collection("carts").add(data).addOnCompleteListener(new OnCompleteListener<DocumentReference>() {
            @Override
            public void onComplete(@NonNull Task<DocumentReference> task) {
                if (task.isSuccessful()) {
                    Toast.makeText(getApplicationContext(),"Add Product Successfully!!",Toast.LENGTH_SHORT).show();
                    Log.i("Update", "Add Product Successfully");
                    finish();
                }
            }
            ...
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(ProductInfoActivity.this,"Add Product Failed!!",Toast.LENGTH_SHORT).show();
                Log.i("Error", "Add product failed",e);
            }
        });
    ...
    private void addToCart() {
        ...
        productCart.setProductDocId(docId);
        productCart.setQty(qty);
        Map<String,Object> data = new HashMap<>();
//                data.put("createdDt",FieldValue.serverTimestamp());
        data.put("productDocId",docId);
        data.put("productId",productId);
        data.put("qty",qty);


//                productCart.setCreatedDt((Timestamp) FieldValue.serverTimestamp());
        docRef.update("productList", FieldValue.arrayUnion(data)).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
